...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (currentCredit < creditAmount) {
      console.warn('Not enough credits for user:', userId, 'Current:', currentCredit, 'Required:', creditAmount)
    ...
    console.log('Attempting to deduct credit. New count will be:', newCreditCount)
    ...
    return new Response(JSON.stringify({ message: 'Credit deducted successfully', new_credit_count: newCreditCount }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
  } catch (error) {
